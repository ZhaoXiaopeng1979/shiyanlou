...
def co2(dataset):
    data = pd.read_excel(dataset, sheetname='Data')
    ...
    countries =  pd.read_excel(dataset, sheetname='Country')
    ...
    df = pd.concat([data, countries], axis=1)

    df_sum = df.groupby('Income group').sum()
    df_max = df.sort_values(by='Sum emissions', ascending=False).groupby('Income group').head(1)
    df_max = df_max.set_index('Income group')
    ...
    df_min = df_min.set_index('Income group')
